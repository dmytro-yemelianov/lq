 ...
 * chdir.c — POSIX chdir().
 *
 * Sends TM_REQ_CHDIR to taskman with the path bytes in msg[4..].
 * taskman validates (v0.7: absolute paths only, length bounds) and
 * stores into the per-process cwd in tm_process_t.cwd; getcwd reads
 * it back from there.
 */
...
int chdir(const char *path)
{
    ...
    if (plen == 0 || plen >= 256) { qsoe_errno = ENAMETOOLONG; return -1; }
    ...
    seL4_MessageInfo_t tag = seL4_MessageInfo_new(TM_REQ_CHDIR, 0, 0, nwords);
