...
/* optimised getsc_bn() */
#define o_getsc()                                                                                  \
    (*source->str != '\0' && *source->str != '\\' && !backslash_skip ? *source->str++ : getsc_bn())
/* optimised getsc_uu() */
...
#if defined(QSH_SMALL) && !defined(QSH_SMALL_BUT_FAST)
...
Again:
    ...
    while ((c = getsc())) {
        ...
        case SADELIM:
            if ((unsigned int)c == ORD('('))
                ...
                statep->nparen--;
                     ...
                     ((unsigned int)c == ORD(/*{*/ '}') || c == (int)statep->ls_adelim.delimiter)) {
                *wp++ = ADELIM;
                *wp++ = c;
                if ((unsigned int)c == ORD(/*{*/ '}') || --statep->ls_adelim.num == 0)
                    POP_STATE();
                if ((unsigned int)c == ORD(/*{*/ '}'))
                    POP_STATE();
                break;
                ...
                if ((unsigned int)c == ORD('(' /*)*/)) {
                    ...
                    *wp++ = '{' /*}*/;
                        ...
                        *wp++ = c;
                        ...
                        if ((unsigned int)c == ORD(':')) {
                            ...
                            *wp++ = ':';
                            ...
                            statep->nparen = 0;
                            break;
                        } else if (ctype(c, C_ALNUX | C_DOLAR | C_SPC) || c == '(' /*)*/) {
                            /* substring subst. */
                            if (c != ' ') {
                                *wp++ = CHAR;
                                *wp++ = ' ';
                    ...
                    if (ctype(c, C_SUB2)) {
                        ...
                        ungetsc(c);
                        if (state == SDQUOTE || state == SQBRACE)
                            PUSH_STATE(SQBRACE);
                        else
                            PUSH_STATE(SBRACE);
        ...
        case SSQUOTE:
            if ((unsigned int)c == ORD('\'')) {
                POP_STATE();
                if ((statep->ls_flags & LS_HEREDOC) || state == SQBRACE)
                    goto store_char;
                *wp++ = CQUOTE;
                ignore_backslash_newline--;
            } else {
        ...
        case SQBRACE:
            if ((unsigned int)c == ORD('\\')) {
                 ...
                 */
                if ((unsigned int)(c = getsc()) == ORD('"') || (unsigned int)c == ORD('\\') ||
                    ctype(c, C_DOLAR | C_GRAVE) || (unsigned int)c == ORD(/*{*/ '}'))
                    goto store_qchar;
                goto heredocquote;
            }
            goto common_SQBRACE;
...
Done:
    ...
    dp = Xstring(ws, wp);
    if (state == SBASE && ((c == '&' && !Flag(FSH) && !Flag(FPOSIX)) || ctype(c, C_ANGLE)) &&
        ((c2 = Xlength(ws, wp)) == 0 || (c2 == 2 && dp[0] == CHAR && ctype(dp[1], C_DIGIT)))) {
        struct ioword *iop = alloc(sizeof(struct ioword), ATEMP);

        iop->unit = c2 == 2 ? qsh_numdig(dp[1]) : c == '<' ? 0 : 1;
        ...
        c2 = getsc();
        /* <<, >>, <> are ok, >< is not */
        if (c == c2 || ((unsigned int)c == ORD('<') && (unsigned int)c2 == ORD('>'))) {
            iop->ioflag |= c == c2 ? ((unsigned int)c == ORD('>') ? IOCAT : IOHERE) : IORDWR;
        ...
        if (((unsigned int)c == ORD('|')) || ((unsigned int)c == ORD('&')) ||
            ...
            while (p < herep)
                if ((*p)->ioflag & IOHERESTR)
                    ...
                    yyerror(Tf_heredoc, evalstr((*p)->delim, 0));
        }
        return (c);
    }
    ...
    *wp++ = EOS;
    ...
    if (*ident != '\0' && (cf & (KEYWORD | ALIAS))) {
        ...
        if ((cf & KEYWORD) && (p = ktsearch(&keywords, ident, h)) &&
            (!(cf & ESACONLY) || p->val.i == ESAC || (unsigned int)p->val.i == ORD(/*{*/ '}'))) {
...
heredoc_parse_char:
    ...
    if (!*eofp) {
        ...
        case 0:
             ...
             * newlines (qsh -c '...') will work as well.
             */
        case ORD('\n'):
    ...
    while ((c = ord(*s->str++)) == 0) {
        ...
        case SALIAS:
            if (s->flags & SF_ALIASEND) {
                ...
                s = source;
            } else if (*s->u.tblp->val.s && ctype((c = strnul(s->u.tblp->val.s)[-1]), C_SPACE)) {
                /* pop source stack */
                source = s = s->next;
                /*
...
getsc_line(Source *s)
{
    ...
    bool have_tty = interactive && (s->flags & SF_TTY);
    ...
    *xp = '\0';
    ...
    if (have_tty && qsh_tmout) {
        qsh_tmout_state = TMOUT_READING;
        alarm(qsh_tmout);
    }
    if (interactive) {
        if (cur_prompt == PS1)
    ...
    /* QRV: line editor is unconditionally on for ttys.  No mode toggle. */
    if (have_tty) {
        xp = x_read(xp);
        *xp = '\0';
        ...
        while (/* CONSTCOND */ 1) {
            ...
            if (!p && shf_error(s->u.shf)) {
                ...
                kwarnf(KWF_ERR(128) | KWF_VERRNO | KWF_PREFIX | KWF_FILELINE | KWF_BUILTIN |
                           KWF_ONEMSG | KWF_BIUNWIND,
                       shf_errno(s->u.shf), Trderr);
                unwind(LRDERR);
            }
            if (!p || (xp = p, xp[-1] == '\n'))
                break;
...
#if !defined(QSH_SMALL) && HAVE_PERSISTENT_HISTORY
        else
...
set_prompt(int to, Source *s)
{
    ...
    case PS1:
         ...
         */
        {
            ...
            shf = shf_sopen(NULL, strlen(ps1) * 2, SHF_WR | SHF_DYNAMIC, NULL);
            /*
             * Pass 1: process bash-style backslash escapes that depend
             * on runtime state.  \w expands to current_wd, \$ expands
             * to '#' for root (euid==0) or '$' for any other user, \\
             * is a literal backslash.  Other \X is left alone.  Then
             * the original ! and !! line-number substitution runs.
             */
            while ((ch = *ps1++)) {
                if (ch == '\\' && *ps1) {
                    switch (*ps1) {
                    case 'w':
                        shf_puts(current_wd && current_wd[0] ?
                                 current_wd : "?", shf);
                        ps1++;
                        continue;
                    case '$':
                        shf_putc(qsheuid == 0 ? '#' : '$', shf);
                        ps1++;
                        continue;
                    case '\\':
                        shf_putc('\\', shf);
                ...
                if (ch != '!' || *ps1++ == '!')
                    ...
                    --ps1;
                    shf_fprintf(shf, Tf_lu, s ? (unsigned long)s->line + 1 : 0UL);
                }
            }
            ps1 = shf_sclose(shf);
...
pprompt(const char *cp, int ntruncate)
{
    ...
    while (*cp) {
        if (indelimit && *cp != delimiter)
            ...
            if (doprint && (indelimit || (ntruncate < (x_cols * lines + columns))))
 ...
 * Returns true if matching bracket found, false if EOF or newline was found.
