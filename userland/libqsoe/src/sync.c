                   ...
                   const struct _sync_attr *attr)
{
    ...
    s->owner = 0;
    s->count = 0;
    if (type == QRV_SYNC_SEM && attr && attr->count > 0) {
        s->count = attr->count;
    }
    (void)type;
    return 0;
...
int SyncMutexLock(sync_t *s)
{
    ...
    for (;;) {
        ...
        unsigned long tid_only = val & QRV_SYNC_TID_MASK;
        ...
        if (tid_only == 0) {
            ...
            unsigned long want = my_tid | (val & QRV_SYNC_WAITERS);
        ...
        if (!(val & QRV_SYNC_WAITERS)) {
            unsigned long want = val | QRV_SYNC_WAITERS;
            if (!__atomic_compare_exchange_n(&s->owner, &val, want,
                                              0, __ATOMIC_RELAXED,
                                              __ATOMIC_RELAXED))
...
int SyncMutexUnlock(sync_t *s)
{
    ...
    if ((val & QRV_SYNC_TID_MASK) != my_tid) {
        qsoe_errno = EPERM;
        return -1;
    }
    if (s->count > 0) {
