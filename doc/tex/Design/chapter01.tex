...
that combines the formally verified seL4 microkernel with a
QNX-style programming model.  The result is a system that provides
...
\begin{tabular}{@{}ll@{}}
    ...
    Receive ID (rcvid)               & Badge value on incoming message \\
    Channel ownership token          & Master Endpoint capability held by \texttt{taskman} \\
...
Table~\ref{tab:mapping} describes the \emph{mechanism}: which seL4
primitive realises each QNX concept.  It does not describe the
\emph{policy}: who holds which capability, who allocates, who revokes.
That separation is deliberate.  The policy --- in particular, the split
of state between the per-process QSOE C library and the central
\texttt{taskman} server --- is the subject of Chapter~\ref{chap:libqsoe},
\S\ref{sec:state-split}.
